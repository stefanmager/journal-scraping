...
# However, keep in mind that you might be blocked by the sage website if you 
...
def start_scraping():
    scrape_articles(url='https://journals.sagepub.com/toc/JIN/current', outlet="jit")
    scrape_articles(url='https://journals.sagepub.com/toc/JOM/current', outlet="jom")
...
def scrape_articles(url, outlet, skip=False):
    ...
    issue_items = soup.findAll(class_="articleEntry")
    skip_first = skip
    for i in issue_items:
        ...
        h5_title = i.find(class_="hlFld-Title")
        current_article_titles = h5_title.text
        print(current_article_titles)

        all_authors = i.findAll(class_="header")
        ...
        current_article_links = "https://journals.sagepub.com" + h5_link.attrs["href"]
